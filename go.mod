module github.com/FrankieFinancial/ecs-exporter

go 1.12

require (
	github.com/Sirupsen/logrus v0.0.0-20170822132746-89742aefa4b2
	github.com/aws/aws-sdk-go v0.0.0-20170906173017-58370dfb7321
	github.com/beorn7/perks v0.0.0-20160804104726-4c0e84591b9a
	github.com/go-ini/ini v0.0.0-20170813052230-c787282c39ac
	github.com/golang/mock v0.0.0-20170822214903-18f6dd13ccdd
	github.com/golang/protobuf v0.0.0-20170902000452-17ce1425424a
	github.com/jmespath/go-jmespath v0.0.0-20160803190731-bd40a432e4c7
	github.com/matttproud/golang_protobuf_extensions v1.0.1
	github.com/prometheus/client_golang v0.0.0-20170907150625-671c87b04728
	github.com/prometheus/client_model v0.0.0-20170216185247-6f3806018612
	github.com/prometheus/common v0.0.0-20170830190555-bc8b88226a12
	github.com/prometheus/procfs v0.0.0-20170703101242-e645f4e5aaa8
	golang.org/x/crypto v0.0.0-20170825220121-81e90905daef
	golang.org/x/sys v0.0.0-20170906000021-9aade4d3a3b7
)
